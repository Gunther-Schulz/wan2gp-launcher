...
cleanup_cache_wrapper() {
    cleanup_cache "${WEBUI_DIR}" "${LOCAL_TEMP_DIR}" "${CACHE_SIZE_THRESHOLD}" "${AUTO_CACHE_CLEANUP}" "${FORCE_CACHE_CLEANUP}"
}
...
EOF
    ...
    printf "\n%s\n" "${delimiter}"
    printf "${GREEN}Linking forge_content directories...${NC}\n"
    printf "%s\n" "${delimiter}"
    ...
    if [[ -n "$MODELS_DIR" ]]; then
        ...
        # Link into MODELS_DIR structure (forge_content is source of truth) (with safety validation)
        link_content_directory "${content_root}/Stable-diffusion" "${MODELS_DIR}/Stable-diffusion" "Models directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/Lora" "${MODELS_DIR}/Lora" "LoRAs directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/VAE" "${MODELS_DIR}/VAE" "VAE directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/text_encoder" "${MODELS_DIR}/text_encoder" "Text encoder directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/ControlNet" "${MODELS_DIR}/ControlNet" "ControlNet directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/ControlNetPreprocessor" "${MODELS_DIR}/ControlNetPreprocessor" "ControlNet preprocessor directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/ESRGAN" "${MODELS_DIR}/ESRGAN" "Upscalers directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/Codeformer" "${MODELS_DIR}/Codeformer" "CodeFormer directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/GFPGAN" "${MODELS_DIR}/GFPGAN" "GFPGAN directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/diffusers" "${MODELS_DIR}/diffusers" "Diffusers directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/adetailer" "${MODELS_DIR}/adetailer" "ADetailer directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/hypernetworks" "${MODELS_DIR}/hypernetworks" "Hypernetworks directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/deepbooru" "${MODELS_DIR}/deepbooru" "Deepbooru directory" "${content_root}" "${MODELS_DIR}"
        link_content_directory "${content_root}/karlo" "${MODELS_DIR}/karlo" "Karlo directory" "${content_root}" "${MODELS_DIR}"
        
        # Embeddings go to WebUI root regardless (not in models/) (with safety validation)
        link_content_directory "${content_root}/embeddings" "${WEBUI_DIR}/embeddings" "Embeddings directory" "${content_root}" "${WEBUI_DIR}"
    else
        printf "${BLUE}Using WebUI default locations${NC}\n"
        
        # Link into WebUI default structure (with safety validation)
        link_content_directory "${content_root}/Stable-diffusion" "${WEBUI_DIR}/models/Stable-diffusion" "Models directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/Lora" "${WEBUI_DIR}/models/Lora" "LoRAs directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/embeddings" "${WEBUI_DIR}/embeddings" "Embeddings directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/VAE" "${WEBUI_DIR}/models/VAE" "VAE directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/text_encoder" "${WEBUI_DIR}/models/text_encoder" "Text encoder directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/ControlNet" "${WEBUI_DIR}/models/ControlNet" "ControlNet directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/ControlNetPreprocessor" "${WEBUI_DIR}/models/ControlNetPreprocessor" "ControlNet preprocessor directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/ESRGAN" "${WEBUI_DIR}/models/ESRGAN" "Upscalers directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/Codeformer" "${WEBUI_DIR}/models/Codeformer" "CodeFormer directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/GFPGAN" "${WEBUI_DIR}/models/GFPGAN" "GFPGAN directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/diffusers" "${WEBUI_DIR}/models/diffusers" "Diffusers directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/adetailer" "${WEBUI_DIR}/models/adetailer" "ADetailer directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/hypernetworks" "${WEBUI_DIR}/models/hypernetworks" "Hypernetworks directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/deepbooru" "${WEBUI_DIR}/models/deepbooru" "Deepbooru directory" "${content_root}" "${WEBUI_DIR}"
        link_content_directory "${content_root}/karlo" "${WEBUI_DIR}/models/karlo" "Karlo directory" "${content_root}" "${WEBUI_DIR}"
    fi
...
# Link all forge_content directories (models, loras, embeddings, etc.)
sync_forge_content
...
prepare_tcmalloc "${ENV_NAME}" "${CONDA_EXE}" "${ENABLE_TCMALLOC}" "2.34"
